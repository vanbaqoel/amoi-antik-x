...
function view_antrian(id)
{
  //Ajax Load data from ajax
  $.ajax({
    url : document.location.protocol + "//" + document.location.host + "/amoi-antik/antrian/edit_antrian/" + id,
    type: "GET",
    dataType: "JSON",
...
{
  window.location = document.location.protocol + "//" + document.location.host + "/amoi-antik/antrian/ru/" + id;
}
...
function delete_antrian(id)
{
  if(confirm('Apakah Anda yakin mau menghapus data ini?'))
  {
    ...
    $.ajax({
      url : document.location.protocol + "//" + document.location.host + "/amoi-antik/antrian/delete_antrian/" + id,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
        alert("Data berhasil dihapus...");
        antrian_table.ajax.reload();
      },
      error: function (jqXHR, textStatus, errorThrown)
      {
...
function printElement(elem)
{
  ...
  $printSection.innerHTML = "<h3>&nbsp;&nbsp;DATA DETAIL MESIN ANTRIAN</h3>";
  ...
  antrian_table =
    $('#dynamic-table').DataTable({
      serverSide: false,
        ...
        ajax: {
            url: document.location.protocol + "//" + document.location.host + "/amoi-antik/antrian/get_all", /* Populate data using a method in controller */
            type: "POST"
        },
      autoWidth: false,
      scrollX: true,
    ...
    new $.fn.dataTable.Buttons(antrian_table, {
      buttons: [
        {
          ...
          action: function ( e, dt, node, config ) {
                    window.location = document.location.protocol + "//" + document.location.host + "/amoi-antik/antrian/ru/add";
          }
        },
        {
          extend: "excel",
    ...
    antrian_table.buttons().container().appendTo($('.tableTools-container'));
