...
{
  window.location = document.location.protocol + "//" + document.location.host + "/amoi-antik/absensi/ru/" + id;
}
...
function delete_absensi(id)
{
  if(confirm('Apakah Anda yakin mau menghapus data ini?'))
  {
    ...
    $.ajax({
      url : document.location.protocol + "//" + document.location.host + "/amoi-antik/absensi/delete_absensi/" + id,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
...
function printElement(elem)
{
  ...
  $printSection.innerHTML = "<h3>&nbsp;&nbsp;DATA DETAIL MESIN ABSENSI</h3>";
  ...
  absensi_table =
    $('#dynamic-table').DataTable({
      serverSide: false,
        ...
        ajax: {
            url: document.location.protocol + "//" + document.location.host + "/amoi-antik/absensi/get_all", /* Populate data using a method in controller */
            type: "POST"
        },
      autoWidth: false,
      scrollX: true,
    ...
    new $.fn.dataTable.Buttons(absensi_table, {
      buttons: [
        {
          ...
          action: function ( e, dt, node, config ) {
                    window.location = document.location.protocol + "//" + document.location.host + "/amoi-antik/absensi/ru/add";
          }
        },
        {
          extend: "excel",
    ...
    absensi_table.buttons().container().appendTo($('.tableTools-container'));
