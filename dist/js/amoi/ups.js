...
function view_ups(id)
{
  ...
  $.ajax({
    ...
    success: function(data)
    {
      ...
      $('#txtSN').text(data.sn);
      $('#txtOutput').text(data.output + " KVA");
      $('#txtHalfLoad').text(data.half_load + " MENIT");
      $('#txtFullLoad').text(data.full_load + " MENIT");
...
{
  window.location = document.location.protocol + "//" + document.location.host + "/amoi-antik/ups/ru/" + id;
}

function delete_ups(id)
{
  bootbox.confirm(
    ...
    function (result)
    {
      if (result) {
        ...
        $.ajax({
          url : document.location.protocol + "//" + document.location.host + "/amoi-antik/ups/delete_ups/" + id,
          type: "POST",
          dataType: "JSON",
          success: function(data)
          {
...
function printElement(elem)
{
  ...
  $printSection.innerHTML = "<h3>&nbsp;&nbsp;DATA DETAIL UPS</h3>";
...
$(document).ready(function () {
  ...
  ups_table =
    $('#dynamic-table').DataTable({
      ...
      ajax: {
        url: document.location.protocol + "//" + document.location.host + "/amoi-antik/ups/get_all", /* Populate data using a method in controller */
        type: "POST"
      },
      autoWidth: false,
      scrollX: true,
  ...
  new $.fn.dataTable.Buttons(ups_table, {
    buttons: [
      {
        ...
        action: function ( e, dt, node, config ) {
          window.location = document.location.protocol + "//" + document.location.host + "/amoi-antik/ups/ru/add";
        }
      },
      {
        extend: "excel",
  ...
  ups_table.buttons().container().appendTo($('.tableTools-container'));
  ...
  if (sk == '000') {ups_table.button('0').disable();}
