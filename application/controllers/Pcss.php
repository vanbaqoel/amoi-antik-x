...
class Pcss extends CI_Controller {
	...
	{
		$this->load->view('pcss_view');
	}

	public function get_all()
	{
		$list = $this->pcss_model->get_all($this->session->kd_unit);
	...
	public function add_pc()
	{
		...
		$insert = $this->pcss_model->add_pc($data);
	...
	{
		$data = $this->pcss_model->get_by_id($id);
	...
	public function update_pc($id)
	{
		...
		$this->pcss_model->update_pc(array('id' => $id), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function delete_pc($id)
	{
		$this->pcss_model->delete_pc($id);
		echo json_encode(array("status" => TRUE));
	}
}
