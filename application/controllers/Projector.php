...
class Projector extends CI_Controller {
	...
	public function __construct()
	{
        ...
        {
	        /* Load database model */
	        $this->load->model('projector_model');
	...
	{
		$this->load->view('projector_view');
	}

	public function get_all()
	{
		$list = $this->projector_model->get_all($this->session->kd_unit);
        ...
        foreach($list as $row){
        	...
        	$rows[] = '<div class="btn-group">
                      <button type="button" class="btn btn-sm btn-default" title="Lihat" onclick="view_projector('."'$id_enc'".')"><i class="fa fa-eye"></i></button>
                      <button type="button" class="btn btn-sm btn-default" title="Ubah" '.($this->session->crud_locked ? 'disabled' : '').' onclick="edit_projector('."'$id_enc'".')"><i class="fa fa-pencil"></i></button>
                      <button type="button" class="btn btn-sm btn-default" title="Hapus" '.($this->session->crud_locked ? 'disabled' : '').' onclick="delete_projector('."'$id_enc'".')"><i class="fa fa-trash"></i></button>
                      </div>';
	...
	public function ru($save_method = NULL)
	{
		...
		$this->load->view('projector_ru_view', $data);
	}

	public function add_projector()
	{
		$data = array(
			'id' => '',
	...
	public function edit_projector($id_enc = NULL)
	{
		...
		$data = $this->projector_model->get_by_id($id);

		echo json_encode($data);
	}

	public function update_projector($id_enc = NULL)
	{
		if ($id_enc === NULL) {
			show_404();
		}
